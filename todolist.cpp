...
void ulozUkoly(const std::vector<Task>& ukoly, const std::string& soubor) {
...
std::vector<Task> nactiUkoly(const std::string& soubor) {
...
void vytiskniUkol(const Task& ukol) {
    std::cout << "ID: " << ukol.id << ", Popis: " << ukol.description << ", Dokonceno: " << (ukol.done ? "Ano" : "Ne") << "\n";
}

void vytiskniUkoly(const std::vector<Task>& ukoly) {
    for (const auto& ukol : ukoly) {
        vytiskniUkol(ukol);
    }
}

bool pridatUkol(std::vector<Task>& ukoly, const std::string& description) {
    int newId = 1;
    if (!ukoly.empty()) {
...
bool odebratUkol(std::vector<Task>& ukoly, int id) {
...
bool oznacitUkolDokonceny(std::vector<Task>& ukoly, int id) {
    for (auto& ukol : ukoly) {
        if (ukol.id == id) {
            ukol.done = true;
            return true;
